...
def load_artifacts(
    preprocessing_path: str, model_path: str
) -> tuple[dict, dict]:
    ...
    try:
        ...
        if (
            preprocessing_artifact["feature_columns"]
...
def _extract_transaction_ids(raw_df: pd.DataFrame) -> pd.Series:
    ...
    try:
        ...
        logger.error(
            "Raw input is missing column 'k' (transaction identifier): %s", e
        )
        raise CustomException(str(e), sys) from e
...
def predict(
    raw_df: pd.DataFrame, preprocessing_artifact: dict, model_artifact: dict
) -> pd.DataFrame:
    ...
    try:
        ...
        features = preprocessing.transform(
            raw_df, preprocessing_artifact["params"]
        )
        features = preprocessing.align_columns(
...
def inference_pipeline(
    raw_df: pd.DataFrame, preprocessing_path: str, model_path: str
) -> pd.DataFrame:
    """End-to-end inference stage: load artifacts and score a batch of new raw transactions."""
    logger.info(
        "Starting inference pipeline: %d rows, preprocessing_path=%s, model_path=%s",
        len(raw_df),
        preprocessing_path,
        model_path,
    )
    try:
        preprocessing_artifact, model_artifact = load_artifacts(
